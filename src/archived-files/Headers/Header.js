...
//                             {
//                                 navItems.map(dat =>
//                                     <Link key={dat.name} to={"/"+dat.path} onClick={()=>setNav(!nav)} className={`flex gap-1 items-center text-indigo-500  no-underline ${dat.path === path ? "text-indigo-700" : "text-black"}`} >{dat.icon} {dat.name}</Link>
//                                 )}
